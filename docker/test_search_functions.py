...
def test_duckduckgo_search_function():
    ...
    # Test the function
    results = fetch_duckduckgo_search_results_test(
        "What is the capital of France?", max_results=1
    )
    ...
    assert (
        result["link"] == "https://en.wikipedia.org/wiki/Paris"
    ), "Link should match mock data"
    assert (
        "Paris is the capital" in result["snippet"]
    ), "Snippet should contain expected content"
...
def test_empty_query_handling():
    ...
    # Test valid query
    assert handle_empty_query("test") == [
        "mock_result"
...
def test_api_key_environment_handling():
    ...
    try:
        ...
        assert should_use_duckduckgo(), "Should use DuckDuckGo when no Serper key"

        # Test with Serper key
        os.environ["SERPER_API_KEY"] = "test_key"  # pragma: allowlist secret
        assert (
            should_use_duckduckgo() is False
        ), "Should use Serper when key is available"
